                  ...
                  {symptoms.length > 0 && <div className="flex flex-wrap gap-4 pt-3 border-t mt-3">{symptoms.map((sym,si)=> 
                    <div key={si} className="flex flex-col items-center p-2 rounded-xl border-2 min-w-[140px] bg-pink-50 border-pink-200">
                      <div className="h-32 w-32 flex items-center justify-center mb-2">
                        {sym.image ? <img src={sym.image} alt={sym.name} className="max-h-full max-w-full object-contain" onError={(e)=>{e.currentTarget.style.display='none'; if(e.currentTarget.parentElement) e.currentTarget.parentElement.innerHTML='<span className="text-7xl">❓</span>';}} /> : <span className="text-7xl">❓</span>}
                      </div>
            ...
            {sos.map((item,i)=>{ 
              ...
              return <div key={i} className="bg-white p-4 rounded-xl shadow border border-teal-200">
                ...
                {sList.length>0 && <div className="flex flex-wrap gap-3 pt-3 border-t mt-2">{sList.map((sym,si)=> 
                  <div key={si} className="flex flex-col items-center p-2 rounded-xl border-2 min-w-[110px] bg-pink-50 border-pink-200">
                    <div className="h-24 w-24 flex items-center justify-center mb-1">{sym.image ? <img src={sym.image} alt={sym.name} className="max-h-full max-w-full object-contain" onError={(e)=>{e.currentTarget.style.display='none'; if(e.currentTarget.parentElement) e.currentTarget.parentElement.innerHTML='<span className="text-5xl">❓</span>';}} /> : <span className="text-5xl">❓</span>}</div>
