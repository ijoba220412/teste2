        ...
        <div className="mb-10 overflow-x-auto">
          <table className="w-full border-collapse border-b-4 border-slate-900">
            ...
            <tbody>{fixed.length === 0 ? <tr><td colSpan={9} className="border p-8 text-center">NENHUM MEDICAMENTO DE USO CONTÍNUO</td></tr> : fixed.map((med, idx) => {
              ...
              return <tr key={idx} className={idx%2===0?'bg-white':'bg-slate-50'}><td className="border p-4 align-top"><div><p className="text-xl font-black uppercase mb-2">{med.nome}</p><div className="flex items-center gap-2"><div className="rounded bg-teal-100 p-2"><MedicationTypeIcon form={med.apresentacao} /></div><p className="text-sm font-bold italic uppercase">{dose} ({med.apresentacao || 'COMPRIMIDO'})</p></div></div>{symptoms.length>0 && <div className="flex flex-wrap gap-3 pt-2 border-t mt-2">{symptoms.map((sym,si)=> <div key={si} className="flex flex-col items-center p-2 rounded-xl border-2 min-w-[100px] bg-pink-50 border-pink-200"><div className="h-16 w-16 flex items-center justify-center">{sym.image ? <img src={sym.image} alt={sym.name} className="max-h-full max-w-full object-contain" onError={(e)=>{e.currentTarget.style.display='none'; if(e.currentTarget.parentElement) e.currentTarget.parentElement.innerHTML='<span class="text-4xl">❓</span>';}} /> : <span className="text-4xl">❓</span>}</div><span className="text-xs font-black text-center text-pink-700">PARA {sym.name.toUpperCase()}</span></div>)}</div>}</td>{SCHEDULE_SLOTS.map(slot => { const active = med.horarios?.some((h:string) => { const hour = parseInt(h.split(':')[0]); if(slot.id==='madrugada') return hour>=0 && hour<6; if(slot.id==='acordar') return hour>=6 && hour<7; if(slot.id==='cafe') return hour>=7 && hour<11; if(slot.id==='almoco') return hour>=11 && hour<14; if(slot.id==='tarde') return hour>=14 && hour<17; if(slot.id==='fim_tarde') return hour>=17 && hour<19; if(slot.id==='jantar') return hour>=19 && hour<21; if(slot.id==='dormir') return hour>=21; return false; }); return <td key={slot.id} className="border p-3 text-center align-middle">{active ? <MedicationVisual dose={dose} form={med.apresentacao} /> : <div className="h-2 bg-slate-100 rounded-full mx-2" />}</td>; })}</tr>;
            })}</tbody>
          </table>
        </div>

        {sos.length > 0 && <div className="mb-10 p-6 rounded-2xl border-4 border-dashed border-teal-500 bg-teal-50"><div className="flex items-center gap-3 mb-6"><div className="rounded-full bg-teal-500 p-3"><History className="h-10 w-10 text-white" /></div><div><h3 className="text-2xl font-black text-teal-900">MEDICAMENTOS SOS</h3><p className="text-base font-bold text-teal-700">TOMAR SOMENTE SE NECESSÁRIO</p></div></div><div className="grid gap-4 sm:grid-cols-2">{sos.map((item,i)=>{ let sList:any[] = []; if(item.symptoms) item.symptoms.forEach((s:any)=>{ let img = s.file ? `/img/n/f/${s.file}` : (s.id ? `/img/n/f/${s.id}.png` : getSymptomImage(s.name||item.indicacao)); sList.push({image: img, name: s.name||item.indicacao||''}); }); else if(item.indicacao) sList.push({image: getSymptomImage(item.indicacao), name: item.indicacao}); return <div key={i} className="bg-white p-4 rounded-xl shadow border border-teal-200"><div className="flex items-start gap-4"><div className="rounded-lg bg-teal-100 p-3"><MedicationTypeIcon form={item.apresentacao} /></div><div><p className="text-xl font-black uppercase">{item.nome}</p><p className="text-sm font-bold uppercase">{item.texto_original_da_posologia}</p></div></div>{sList.length>0 && <div className="flex flex-wrap gap-2 pt-3 border-t mt-2">{sList.map((sym,si)=> <div key={si} className="flex flex-col items-center p-2 rounded-xl border-2 min-w-[90px] bg-pink-50 border-pink-200"><div className="h-14 w-14 flex items-center justify-center">{sym.image ? <img src={sym.image} alt={sym.name} className="max-h-full max-w-full object-contain" onError={(e)=>{e.currentTarget.style.display='none'; if(e.currentTarget.parentElement) e.currentTarget.parentElement.innerHTML='<span class="text-3xl">❓</span>';}} /> : <span className="text-3xl">❓</span>}</div><span className="text-xs font-black text-center text-pink-700">PARA {sym.name.toUpperCase()}</span></div>)}</div>}</div>; })}</div></div>}
